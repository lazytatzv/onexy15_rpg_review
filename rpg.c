...
int main(int argc,char** argv){
    ...
    //
    // NOTE:
    // 誤解を招く可能性があるので一応補足しておくと、
    // variable1, variable2,..を絶対にやってはいけないという訳ではありません.
    // それぞれのケースに対して変数が必要な場面もあります.
    // しかし、今回はいらないですし、大抵の場合はシンプルにできるしした方が良いという話です.
    //
    // 
    // TIPS:
    ...
    while(player1 > 0 || player2 > 0 || player3 > 0 && enm > 0){
        ...
        // FB:
        // これ重要です
        // 早期break/returnできるとnestを浅くできる"可能性があります".
        // Filterするイメージですね. これをガード節(Guard Clause)と呼んだりします.
        // 闇雲に使えば良いものではないですが可読性とのトレードオフでしょうか..
        //
        // このifを通過した時点でplayer1/2/3の誰かが生き残っていることが保証される.
